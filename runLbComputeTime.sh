...
for problem in "${SMALL[@]}"; do 
	for i in {1..5}; do
            for win in ${WINDOWS[@]}; do
                  java -Xmx14g -Xms14g -cp $PROJECTDIR/lib/*: experiments.LbComputeTime $PROJECTDIR $DATASETDIR $problem $RUNS $win LbEnhanced $i
            done
            for r in ${RS[@]}; do
                  java -Xmx14g -Xms14g -cp $PROJECTDIR/lib/*: experiments.LbComputeTime $PROJECTDIR $DATASETDIR $problem $RUNS $r LbEnhanced $i
            done
      done
      for method in ${METHODS[@]}; do 
            for win in ${WINDOWS[@]}; do
                  java -Xmx14g -Xms14g -cp $PROJECTDIR/lib/*: experiments.LbComputeTime $PROJECTDIR $DATASETDIR $problem $RUNS $win $method 5
            done
            for r in ${RS[@]}; do
                  java -Xmx14g -Xms14g -cp $PROJECTDIR/lib/*: experiments.LbComputeTime $PROJECTDIR $DATASETDIR $problem $RUNS $r $method 5
            done
      done
done
