...
LARGE=("UWaveGestureLibraryZ" "Phoneme" "ElectricDevices" "FordB" "FordA" "NonInvasiveFatalECGThorax1"
            "NonInvasiveFatalECGThorax2" "HandOutlines" "UWaveGestureLibraryAll" "StarLightCurves")
...
for problem in "${SMALL[@]}"; do
      for i in "${PARAMS[@]}"; do
            for win in ${WINDOWS[@]}; do
                  java -Xmx30g -Xms30g -cp $PROJECTDIR/lib/*: experiments.PruneAndTimeSortEuclidean $PROJECTDIR $DATASETDIR $problem $RUNS $win false LbEnhanced $i
            done
            for r in ${RS[@]}; do
                  java -Xmx14g -Xms14g -cp $PROJECTDIR/lib/*: experiments.PruneAndTimeSortEuclidean $PROJECTDIR $DATASETDIR $problem $RUNS $r false LbEnhanced $i
            done
      done 
      for method in ${METHODS[@]}; do 
            for win in ${WINDOWS[@]}; do
                  java -Xmx30g -Xms30g -cp $PROJECTDIR/lib/*: experiments.PruneAndTimeSortEuclidean $PROJECTDIR $DATASETDIR $problem $RUNS $win false $method 5
            done
            for r in ${RS[@]}; do
                  java -Xmx14g -Xms14g -cp $PROJECTDIR/lib/*: experiments.PruneAndTimeSortEuclidean $PROJECTDIR $DATASETDIR $problem $RUNS $r false $method 5
            done
      done
done
