...
for problem in "${SMALL[@]}"; do
      for i in {1..5}; do
            for win in ${WINDOWS[@]}; do
                  java -Xmx30g -Xms30g -cp $PROJECTDIR/lib/*: experiments.PruneAndTimeSortLbKeogh $PROJECTDIR $DATASETDIR $problem $RUNS $win false LbEnhanced $i
            done
            for r in ${RS[@]}; do
                  java -Xmx14g -Xms14g -cp $PROJECTDIR/lib/*: experiments.PruneAndTimeSortLbKeogh $PROJECTDIR $DATASETDIR $problem $RUNS $r false LbEnhanced $i
            done
      done 
      for method in ${METHODS[@]}; do 
            for win in ${WINDOWS[@]}; do
                  java -Xmx30g -Xms30g -cp $PROJECTDIR/lib/*: experiments.PruneAndTimeSortLbKeogh $PROJECTDIR $DATASETDIR $problem $RUNS $win false $method 5
            done
            for r in ${RS[@]}; do
                  java -Xmx14g -Xms14g -cp $PROJECTDIR/lib/*: experiments.PruneAndTimeSortLbKeogh $PROJECTDIR $DATASETDIR $problem $RUNS $r false $method 5
            done
      done
done
